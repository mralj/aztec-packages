...
import type { PrivateKey, Secp256k1PeerId, Secp256k1PrivateKey } from '@libp2p/interface';
...
export async function getPeerIdPrivateKey(
  ...
  if (config.peerIdPrivateKey && config.peerIdPrivateKey.getValue().trim()) {
    ...
    return privateKeyFromHex(config.peerIdPrivateKey.getValue().trim());
  }
...
export async function createSecp256k1PeerId(): Promise<Secp256k1PeerId> {
  const privateKey = await generateKeyPair('secp256k1');
  return peerIdFromPrivateKey(privateKey);
}

/** Creates a new libp2p private key. */
export function createSecp256k1PrivateKey(): Promise<Secp256k1PrivateKey> {
...
export async function createSecp256k1PrivateKeyWithPeerId() {
  const privateKey: Secp256k1PrivateKey = await createSecp256k1PrivateKey();
  const peerId: Secp256k1PeerId = peerIdFromPrivateKey(privateKey);
